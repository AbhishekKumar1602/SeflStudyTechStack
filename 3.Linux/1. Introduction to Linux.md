...
```
updatedb
locate /path/to/search -name filename
```
...
- Explanation:
   ...
   The non-swapped physical memory used by the process.
...
**Job Commands**
1. **`jobs`**: 
- The jobs command displays a list of current jobs in the active shell session.
- Example:
```
jobs
```
2. **`fg`**: 
- The fg command brings a background job into the foreground for active execution.
- Example:
   ```
   fg %1
   ```
- Explanation: In this example, %1 refers to the job number 1.

3. **`bg`**: 
- The bg command resumes a stopped or suspended job in the background.
- Example
   ```
   bg %2
   ```
